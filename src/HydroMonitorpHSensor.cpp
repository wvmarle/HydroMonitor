...
void HydroMonitorpHSensor::readSensor() {
  ...
  if (millis() - lastWarned > WARNING_INTERVAL) {
    if (sensorData->pH > 9 || sensorData->pH < 4) {
      lastWarned = millis();
      char message[110];
      sprintf_P(message, PSTR("Measured pH level of %2.2f is unusual, possible malfunction."), sensorData->pH);
      logging->writeError(message);
    }
    else if (sensorData->pH > sensorData->targetpH + 1) {
      lastWarned = millis();
      char message[120];
      sprintf_P(message, PSTR("pH level is too high; correction with pH adjuster is urgently needed.\n"
