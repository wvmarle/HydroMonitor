...
void HydroMonitorWaterTempSensor::begin(HydroMonitorCore::SensorData *sd, HydroMonitorLogging *l, MS5837 *ms) {
  ...
  if (ds18b20->getDeviceCount() > 0) {
    ...
    l->writeTrace(F("HydroMonitorWaterTempSensor: configured DS18B20 sensor."));
  }
  else {
    l->writeError(F("HydroMonitorWaterTempSensor: no DS18B20 sensor found."));
  }  
...
//  reading = reading >> NTCSAMPLES;
  ...
  // Nothing to do here.
    ...
    if (millis() - lastWarned > WARNING_INTERVAL && 
      ...
      lastWarned = millis();
      char message[120];
      sprintf_P(message, PSTR("HydroMonitorWaterTempSensor: unusual temperature value measured: %2.1f °C."), 
