...
void HydroMonitorReservoir::doReservoir() {
  ...
  if (floatswitchTriggered) {
    ...
    if (millis() - lastWarned > WARNING_INTERVAL) {
      logging->writeError(F("Float switch triggered, reservoir water level is critically high."));
      lastWarned = millis();
    }
  }
  else {
  ...
  if (bitRead(sensorData->systemStatus, STATUS_DRAINING_RESERVOIR) ||
    ...
    if (bitRead(sensorData->systemStatus, STATUS_FILLING_RESERVOIR)) { // Reservoir is being filled.
      ...
      if (millis() - startAddWater > 3 * 60 * 1000) {       // As extra safety measure: close the valve after 2 minutes, regardless of what the water level sensor says.
        closeValve();
...
/*
        ...
        if (millis() - lastWarned > WARNING_INTERVAL) {
          lastWarned = millis();
          logging->writeError(F("HydroMonitorReservoir: shortly after the reservoir was topped up, water level dropped below the minimum already."));
        }
      }
*/
  ...
  if (sensorData->waterLevel > settings.maxFill + 5 &&
    ...
    logging->writeError(F("HydroMonitorReservoir: water level >5% over maxFill within 5 minutes of starting to fill the reservoir. Suspected problem with the filling system."));
  }  
